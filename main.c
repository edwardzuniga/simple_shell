	...
	while (1)
	{
		...
		if (getcommand)
		{
			...
			if (!_strcmp(user_command[0], "exit") && user_command[1] == NULL)
				exit_shell(user_command, getcommand, _exit);
			if (!_strcmp(user_command[0], "env"))
				_getenv(env);
			else
			{
				n = tokenizer_path(&user_command[0], env);
				_exit = run_com(user_command, av, env, getcommand, pathValue, n);
				if (n == 0)
					free(user_command[0]);
			}
			free(user_command);
